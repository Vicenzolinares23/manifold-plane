...
pub fn reconcile<C: ClusterClient>(
                ...
                .and_then(|s| s.parse::<f64>().ok())
            {
                Some(requested) if requested > 0.0 => {
                    ...
                    if current_budget > 0.0 && delta.abs() > max_delta {
                        ...
                        actions.push(Action::SetBudget {
                            bits_squared: requested,
                        });
                        budget = requested;
                    }
                }
    ...
    for class in &policy.spec.symmetry_classes {
        ...
        if members < MIN_CLASS_SIZE {
            actions.push(Action::WarnUndersizedClass {
            ...
            actions.push(Action::RegisterClass {
                name: class.name.clone(),
                members,
            });
        }
...
mod tests {
    ...
    fn cal(budget: &str) -> BTreeMap<String, String> {
        [("budgetBitsSquared".to_string(), budget.to_string())]
            .into_iter()
            .collect()
    }
    ...
    fn a_calibrated_policy_with_a_healthy_class_becomes_ready() {
        let c = FakeClient::new()
            .with_config_map("mp", "cal", cal("512"))
            .with_count("ctrl", 20);
    ...
    fn a_large_budget_jump_is_clamped() {
        ...
        let c = FakeClient::new()
            .with_config_map("mp", "cal", cal("10000"))
            .with_count("ctrl", 20);
    ...
    fn an_undersized_symmetry_class_is_reported() {
        ...
        assert!(r
            .actions
            .iter()
            .any(|a| matches!(a, Action::WarnUndersizedClass { members: 2, .. })));
        assert_eq!(r.status.phase, "ReadyWithWarnings");
    ...
    fn alpha_is_recomputed_when_the_budget_changes() {
        ...
        assert!(r
            .actions
            .iter()
            .any(|a| matches!(a, Action::SetAlpha { .. })));
    }
    ...
    fn an_invalid_spec_is_refused_before_any_cluster_write() {
        ...
        assert!(
            c.patches.borrow().is_empty(),
            "must not write status for an invalid spec"
        );
    }
}
